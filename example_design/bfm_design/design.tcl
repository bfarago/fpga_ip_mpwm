proc create_ipi_design { offsetfile design_name } {
	...
	set gpio_io_i [ create_bd_port -dir I gpio_i ]
	set gpio_io_o [ create_bd_port -dir O gpio_o ]
	set gpio_io_t [ create_bd_port -dir O gpio_t ]
	
	# Create instance: mpwm_0, and set properties
	set mpwm_0 [ create_bd_cell -type ip -vlnv xilinx.com:user:mpwm:1.1 mpwm_0]
	...
	connect_bd_intf_net [get_bd_intf_pins master_0/M_AXI ] [get_bd_intf_pins mpwm_0/S00_AXI]

	# Create port connections
	connect_bd_net -net aclk_net [get_bd_ports ACLK] [get_bd_pins master_0/ACLK] [get_bd_pins mpwm_0/S00_AXI_ACLK]
	connect_bd_net -net aresetn_net [get_bd_ports ARESETN] [get_bd_pins master_0/ARESETN] [get_bd_pins mpwm_0/S00_AXI_ARESETN]
...
set_property -name {xsim.simulate.runtime} -value {100ms} -objects [get_filesets sim_1]
	...
	set offset_file "${bd_path}/mpwm_tb_include.svh"
	set fp [open $offset_file "w"]
	puts $fp "`ifndef mpwm_tb_include_vh_"
	puts $fp "`define mpwm_tb_include_vh_\n"
...
set ip_path [file dirname [file normalize [get_property XML_FILE_NAME [ipx::get_cores xilinx.com:user:mpwm:1.1]]]]
set test_bench_file ${ip_path}/example_designs/bfm_design/mpwm_tb.sv
...
remove_files -quiet -fileset sim_1 mpwm_tb_include.vh
import_files -fileset sim_1 -norecurse -force $interface_address_vh_file
set_property top mpwm_tb [get_filesets sim_1]
